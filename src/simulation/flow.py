...
système IViDiS :
    ...
    - Dans les réacteurs agités R1/R2 (CSTR, bien mélangé) : il n'y a pas de direction d'écoulement unique. On utilise plutôt le taux de
      renouvellement du fluide (Q/V), qui correspond à l'inverse du temps de séjour moyen τ = V/Q, et sert à modéliser la probabilité de
...
def cstr_renewal_rate(flow_rate_ml_min: float, volume_ml: float) -> float:
    ...
    Correspond à l'inverse du temps de séjour moyen tau = V/Q
...
def mean_residence_time_cstr(flow_rate_ml_min: float, volume_ml: float) -> float:
    """
    Temps de séjour moyen tau = V/Q (s) d'un réacteur agité, référence théorique CSTR, utile pour valider qualitativement le comportement du modèle.
    """
